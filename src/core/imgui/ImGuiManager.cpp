...
ImGuiManager::ImGuiManager()
    ...
    // fonts - order matters - the ImGuiColorTextEdit widget is weird...
    big = io.Fonts->AddFontFromFileTTF(
            (Utils::getPathToExe() + "../../../third_party/imgui/misc/fonts/Cousine-Regular.ttf")
                    .c_str(),
            20.f);
    main = io.Fonts->AddFontDefault();
    hassert(main);
    hassert(big);
...
ImFont* ImGuiManager::getMainFont() const { return main; }
ImFont* ImGuiManager::getBigFont() const { return big; }
...
void ImGuiManager::imguiRenderCallback(ImDrawData* data) {
    ...
    auto        projection = matrix::ortho_rh(0, w, h, 0, 0,
                                       1); // note the inverted height. ImGui uses 0,0 as top left
    ...
    glVertexAttribPointer(Attrib_Position, 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert),
                          HA_OFFSET_OF(ImDrawVert, pos));
    glVertexAttribPointer(Attrib_TexCoord, 2, GL_FLOAT, GL_FALSE, sizeof(ImDrawVert),
                          HA_OFFSET_OF(ImDrawVert, uv));
    glVertexAttribPointer(Attrib_Color, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(ImDrawVert),
                          HA_OFFSET_OF(ImDrawVert, col));

    for(int i = 0; i < data->CmdListsCount; ++i) {
        const auto list = data->CmdLists[i];

        glBufferData(GL_ARRAY_BUFFER, sizeof(ImDrawVert) * list->VtxBuffer.size(),
                     list->VtxBuffer.Data, GL_DYNAMIC_DRAW);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(ImDrawIdx) * list->IdxBuffer.size(),
                     list->IdxBuffer.Data, GL_DYNAMIC_DRAW);
        ...
        for(const auto& cmd : list->CmdBuffer) {
            if(cmd.UserCallback) {
                ...
                glScissor(int(cmd.ClipRect.x), (int)(h - cmd.ClipRect.w),
